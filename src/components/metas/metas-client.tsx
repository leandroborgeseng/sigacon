      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <Button variant="outline" onClick={() => baixar("csv")}>CSV</Button>
            <Button variant="outline" onClick={() => baixar("ods")}>ODS</Button>
            {podeEditar && <Button onClick={bootstrap2026}>Carregar metas-base 2026</Button>}
          </div>

          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            <Card><CardContent className="pt-6"><p className="text-xs text-muted-foreground">Metas</p><p className="text-2xl font-bold">{indicadores.totalMetas}</p></CardContent></Card>
            <Card><CardContent className="pt-6"><p className="text-xs text-muted-foreground">Desdobramentos</p><p className="text-2xl font-bold">{indicadores.totalDesdobramentos}</p></CardContent></Card>
            <Card><CardContent className="pt-6"><p className="text-xs text-muted-foreground">Concluídos</p><p className="text-2xl font-bold">{indicadores.totalConcluidos}</p></CardContent></Card>
            <Card><CardContent className="pt-6"><p className="text-xs text-muted-foreground">% médio</p><p className="text-2xl font-bold">{indicadores.percMedio.toFixed(1)}%</p></CardContent></Card>
          </div>
            ...
            <div className="space-y-1">
              ...
              <Select value={filtroStatus} onValueChange={(v) => setFiltroStatus(v as "TODOS" | StatusMeta)}>
                ...
                <SelectContent>
                  ...
                  {Object.values(StatusMeta).map((s) => <SelectItem key={s} value={s}>{STATUS_LABEL[s]}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              ...
              <Select value={filtroResponsavel} onValueChange={setFiltroResponsavel}>
                ...
                <SelectContent>
                  <SelectItem value="TODOS">Todos</SelectItem>
                  {responsaveisOptions.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
        ...
        <Card>
          ...
          <CardContent className="grid gap-2">
            ...
            <div className="flex gap-2 items-end">
              <div className="w-44 space-y-1"><Label>Prazo</Label><Input type="date" value={novaMeta.prazo} onChange={(e) => setNovaMeta((v) => ({ ...v, prazo: e.target.value }))} /></div>
