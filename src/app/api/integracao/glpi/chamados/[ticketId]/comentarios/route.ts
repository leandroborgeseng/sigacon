...
import { glpiCreateTicketFollowup, glpiWithSession } from "@/lib/glpi-client";
...
export async function POST(request: Request, ctx: { params: Promise<{ ticketId: string }> }) {
  ...
  if (!content) {
    return NextResponse.json({ message: "Informe o comentário" }, { status: 400 });
  }

  try {
    const result = await glpiWithSession((s) =>
      glpiCreateTicketFollowup(s, idNum, { content, privado: Boolean(body.privado) })
    );
    return NextResponse.json({ ok: true, ...result });
  } catch (e) {
    return NextResponse.json(
      { ok: false, message: e instanceof Error ? e.message : "Erro ao criar comentário no GLPI" },
      { status: 502 }
    );
  }
}
